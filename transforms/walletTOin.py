...
@registry.register_transform(display_name="Input Transactions",
                             input_entity="hawkdev.BitcoinWallet",
                             description="Get input transactions for a wallet",
                             output_entities=["maltego.BTCTransaction"],
                             transform_set=WalletExplorer_set)
class walletTOin(DiscoverableTransform):
    ...
    def create_entities(cls, request, response):
        ...
        try:
            transactions = wallet.inputTransactions()
        except Exception as e:
